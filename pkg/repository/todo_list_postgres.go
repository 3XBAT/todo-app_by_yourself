...
func (r *TodoListPostgres) Create(userId int, list todo.TodoList) (int, error) {
	...
	var id int
	createListQuery := fmt.Sprintf("INSERT INTO %s (title, description) VALUES ($1, $2)", usersListsTable)
...
func (r *TodoListPostgres) Update(userId, listId int, input todo.UpdateListInput) error {
	...
	query := fmt.Sprintf("UPDATE %s tl SET %s FROM %s ul WHERE tl.id = ul.list_id AND ul.id = $%d AND ul.user_id = $%d",
		todoListsTable,usersListsTable, setQuery, argId, argId+1)
